  ...
  switch (type) {
    ...
    case WStype_TEXT:                     // if new text data is received
      ...
      if (payload[0] == '#') {            // we get RGB data
        ...
        analogWrite(LED_RED,  255);    // Write the right color to the LED output pins
        analogWrite(LED_GREEN, 0);
        analogWrite(LED_BLUE, 0);
      } else if (payload[0] == 'G') {                      // the browser sends an N when the rainbow effect is disabled
        analogWrite(LED_RED,   0);    // Write the right color to the LED output pins
